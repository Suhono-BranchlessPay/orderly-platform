...
| Step 5 menu / catalog | **Done (this branch)** — `wizard.catalog` SKU prefix (+ reserved/live uniqueness check) + ambiguous-name review + prices/modifiers confirms; optional Square catalog preview; full `menu_items` sync still at publish (Blok A) |
